...
def _get_mongodb_hosts(stack):
    ...
    for mongodb_host in mongodb_hosts:
        ...
        # insert volume_name 
        # ref 45304958324
        _volume_name = "{}-{}".format(mongodb_host,
                                      stack.volume_mountpoint).replace("/","-").replace(".","-")
...
def run(stackargs):
    ...
    stack.parse.add_optional(key="mongodb_data_dir",
                             default="/var/lib/mongodb")

    stack.parse.add_optional(key="mongodb_storage_engine",
                             default="wiredTiger")

    stack.parse.add_optional(key="mongodb_version",
                             default="4.2")

    stack.parse.add_optional(key="mongodb_port",
                             default="27017")

    stack.parse.add_optional(key="mongodb_bind_ip",
                             default="0.0.0.0")

    stack.parse.add_optional(key="mongodb_logpath",
    ...
    stack.add_hostgroups("config0-publish:::mongodb::ubuntu_vendor_setup",
                         "ubuntu_vendor_setup")

    stack.add_hostgroups("config0-publish:::mongodb::ubuntu_vendor_init_replica",
    ...
    base_env_vars = {"METHOD": "create",
                     ...
                     "ANS_VAR_mongodb_main_ips": "{},{}".format(public_ips[0],
                                                                private_ips[0]),
